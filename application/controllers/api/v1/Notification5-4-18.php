...
class Notification extends REST_Controller {
    ...
    public function parent_post() {
        ...
        if ($is_valid) {
            ...
            try {
                ...
                if (!empty($data)) {
                    foreach ($data as $k => $v) {
                        if (isset($data[$k]['device_os']) == 'android' || isset($data[$k]['device_os']) == 'Android')
                            $result = $this->send($fcmMsg, $data[$k]['device_token']);
                        if (isset($data[$k]['device_os']) == 'ios')
    ...
    public function isend($data, $devicetoken) {
       ...
       $deviceToken = $devicetoken;
		...
		stream_context_set_option($ctx, 'ssl', 'local_cert', 'noezee-pushcert.pem');
