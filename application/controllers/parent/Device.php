...
// COUNT TOTAL NUMBER OF ROWS IN TABLE
            ...
            $total_rows = $this->device_model->get_devicedetail_count($userId);
...
// DETERMINE NUMBER OF PAGES
            ...
            $data['page'] = $pages;
            $data['joblisting'] = $this->device_model->get_mediadetails($offset, $limit, $userId);
    ...
    public function media($type, $deviceId, $pageNo = 0) {
        ...
        if (!isset($this->session->userdata['ud'])) {
            ...
            $page = array();
            $data = array();
            $config['base_url'] = BASE_URL . 'parent/gallery/' . $type . '/' . $deviceId;
            $limit = RECORD_LIMIT;
...
// COUNT TOTAL NUMBER OF ROWS IN TABLE
            ...
            $total_rows = $this->device_model->get_devicemediadetail_count($deviceId, $type);
            //$config['page_query_string'] = TRUE;
            //$config['reuse_query_string'] = TRUE;
            if ($total_rows > $limit) {
                $number_of_pages = ceil($total_rows / $limit);
            ...
            $data['joblisting'] = $this->device_model->get_devicemediadetails($offset, $limit, $deviceId, $type);
            // $data['technicianList'] = $this->jobs->get_technicianlist($page, $config['per_page'], $data['jobDetails'][0]['category_id']);
            /* Load the view using template */
            $data['header'] = $this->load->view('elements/header', $page, true);
            //$data['nav'] = $this->load->view('elements/navigation', $page, true);
    ...
    public function favorite($deviceId, $date = '', $pageNo = 0) {
        ...
        $this->load->model('parent/favorite_model');
        $data = array();
...
// COUNT TOTAL NUMBER OF ROWS IN TABLE
            ...
            $total_rows = $this->favorite_model->get_count_media($filterData);
            if ($total_rows > $limit) {
                $number_of_pages = ceil($total_rows / $limit);
                $pages = $pageNo;
            } else {
            ...
            //$data['project'] = $this->media->get_project(0);
            // $data['neighborhood'] = $this->media->get_neighborhood();
            $data["joblisting"] = $this->favorite_model->get_media('', $offset, $limit, $filterData, '');
