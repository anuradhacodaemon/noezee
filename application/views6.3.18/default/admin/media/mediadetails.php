            ...
            <div class="">
                <div class="page-title">
                    <div class="title_left">
                        ...
                        <a href="<?php echo base_url() . 'admin' ?>" class="dark_grey">Home</a> >  <a href="<?php echo base_url() . 'admin/media' ?>" class="dark_grey">Media Listing</a> >Details
...
?>    
        ...
        <script>
            $("#cPrice").click(function () {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url() . MASTERADMIN . '/jobs/confirmprice' ?>",
                    data: $("#antoform2").serialize(), // serializes the form's elements.
                    success: function (data)
                    {
                        $("#fprice").html(data);
<?php if ($jobDetails[0]['job_status'] == 1) { ?>
                            $('#jobStatus').html('Taken by dispatcher');
<?php } ?>
<?php if ($jobDetails[0]['job_status'] == 2) { ?>
                            $('#jobStatus').html('Assigned to technician or architect');
<?php } ?>
                        $('.modal-backdrop').removeClass('modal-backdrop fade in');

                        $("#CalenderModalEdit").attr('class', 'modal fade');
                        $("#CalenderModalEdit").attr("aria-hidden", "true");
                        /*$("#CalenderModalEdit").css('display', 'none');*/
                        $("body").attr('class', 'nav-md  pace-done');
                    }
                });
            });
            $("#cPrice1").click(function () {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url() . MASTERADMIN . '/jobs/confirmreviewprice' ?>",
                    data: $("#antoform3").serialize(), // serializes the form's elements.
                    success: function (data)
                    {

                        $("#fprice").html(data);
                        $('#jobStatus').html('Arrival at the client address');
                        $('.modal-backdrop').removeClass('modal-backdrop fade in');
                        //$("#CalenderModalEdit1").css('display', 'none');
                        $("#CalenderModalEdit1").attr('class', 'modal fade');
                        $("#CalenderModalEdit1").attr("aria-hidden", "true");
                        $("body").attr('class', 'nav-md  pace-done');
                    }
                });
            });
            function getAssign(techid)
            {
                $.ajax({
                    type: "GET",
                    url: "<?php echo base_url() . MASTERADMIN . '/jobs/assigntechnician/' . $jobDetails[0]['job_id'] . '/' ?>" + techid,
                    success: function (data)
                    {
                        //alert(data); // show response from the php script.
                        $('#assignTech').append(data);
                        $('#ListofTechnician').html("");
                    }
                });
            }
        </script>
